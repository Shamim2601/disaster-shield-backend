...
def get_disaster_by_title(db:Session, title:str):
    return db.query(models.Disaster).filter(models.Disaster.title == title).first()


def get_disaster_by_id(db:Session, disaster_id:int):
    return db.query(models.Disaster).get({"disaster_id":disaster_id})
